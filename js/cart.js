...
class ProductsItems {
  ...
  getProductById(id) {
    const method = "find";
    // return this.cart?.[method]((item) => item._id === id);
  }
  ...
  addEventListeners(action, element) {
    switch (action) {
      case "add":
        ...
        break;
      case "cart":
        const buttonsRemoveFromCart =
          element.querySelectorAll(".remove-from-cart");

        const buttonsDecrement = element.querySelectorAll(".btn-decrement");
        const buttonsIncrement = element.querySelectorAll(".btn-increment");

        // Remove from cart
        for (const button of buttonsRemoveFromCart) {
          button.addEventListener("click", (event) => {
            const { productId } = event.target.dataset;
